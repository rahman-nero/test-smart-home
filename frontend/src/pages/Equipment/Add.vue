    ...
    <div class="form-container">
      ...
      <form @submit.prevent v-for="index in form_count" key="index">
        ...
        <div class="mb-3">
          ...
          <input type="text" @input="(e) => setSerialNumber(index - 1, e.target.value)" class="form-control"
                 id="serial_number">
        </div>

        <div class="mb-3">
          <label for="comment" class="form-label">Примечание</label>
          <textarea type="text" @input="(e) => setComment(index - 1, e.target.value)" class="form-control"
                    id="comment"></textarea>
        </div>
      ...
      <button type="submit" class="btn btn-secondary" @click="createForm" style="margin-right: 10px">Добавить еще
      </button>
...
export default {
  ...
  methods: {
    ...
    async send() {
      ...
      try {
        const response = await $host.post('/equipment', {equipments: this.data}, {
          ...getHeaderSnippet(),
        })
